...
CodecContext::CodecContext(AVCodecID codec_id, bool is_decoder) {
    codec_ = const_cast<AVCodec*>(FindCodecById(codec_id, is_decoder));
...
CodecContext::CodecContext(const std::string& codec_name, bool is_decoder) {
    codec_ = const_cast<AVCodec*>(FindCodecByName(codec_name, is_decoder));
    codec_ctx_ = avcodec_alloc_context3(codec_);
    if(!codec_ctx_) {
        throw std::runtime_error("avcodec_alloc_context3() failed");
    }
...
CodecContext::CodecContext(AVStream* stream, bool is_decoder) {
    codec_ = const_cast<AVCodec*>(FindCodecById(stream->codecpar->codec_id, is_decoder));
    codec_ctx_ = avcodec_alloc_context3(codec_);
    if(!codec_ctx_) {
        throw std::runtime_error("avcodec_alloc_context3() failed");
    }
...
AVCodecContext* CodecContext::CreateCodecCtxFromID(AVCodecID codec_id, bool is_decoder, AVCodec** codec) {
    // FindCodecById() 保证返回非空
    const AVCodec* avcodec = FindCodecById(codec_id, is_decoder);
...
AVCodecContext* CodecContext::CreateCodecCtxFromStream(AVStream* stream, bool is_decoder, AVCodec** codec) {
    const AVCodec* avcodec = FindCodecById(stream->codecpar->codec_id, is_decoder);
    *codec = const_cast<AVCodec*>(avcodec);
    AVCodecContext* ctx = avcodec_alloc_context3(*codec);
    if(!ctx) {
        throw std::runtime_error("avcodec_alloc_context3() failed");
...
const AVCodec* CodecContext::FindCodecById(AVCodecID id, bool is_decoder) {
...
const AVCodec* CodecContext::FindCodecByName(const std::string& codec_name, bool is_decoder) {
    const AVCodec* codec = nullptr;
    if (is_decoder) {
    ...
    if(!codec) {
        const char* type = is_decoder ? "decoder" : "encoder";
        throw std::runtime_error("avcodec_find_" + std::string(type) + std::string(" failed: ") + codec_name);
    }
    return codec;
}
...
void CodecContext::SetAudioCodecParameters(AVCodecContext* codec_ctx, int sample_rate, int channels, AVSampleFormat sample_fmt, int bit_rate) noexcept {
    codec_ctx->sample_rate = sample_rate;
    av_channel_layout_default(&codec_ctx->ch_layout, channels);
    if (codec_ctx->ch_layout.order != AV_CHANNEL_ORDER_NATIVE) {
        MLOG_WARN("Channel layout is not native, codec may not work.");
        // 这里不做处理，让 avcodec_open2 抛异常
    }
...
void CodecContext::SetAudioQualityCodecParameters(AVCodecContext* codec_ctx, int flags) noexcept {
    codec_ctx->flags |= flags;
}
